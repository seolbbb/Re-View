...
def evaluate_batch_summary(tool_context: ToolContext) -> Dict[str, Any]:
    """현재 배치의 요약 품질을 평가합니다.

    배치 모드에서 사용됩니다. 현재 배치의 segment_summaries.jsonl을 평가합니다.

    Returns:
        success: 실행 성공 여부
        batch_index: 평가된 배치 인덱스
        result: PASS 또는 FAIL
        score: 평가 점수 (0.0 ~ 1.0)
        can_rerun: 재실행 가능 여부
    """
    ...
    if not batch_summaries.exists():
        return {
            "success": False,
            "error": f"배치 {current_batch_index}의 segment_summaries.jsonl이 없습니다.",
        }
    if not batch_segments.exists():
        return {
            "success": False,
            "error": f"배치 {current_batch_index}의 segments_units.jsonl이 없습니다.",
        }
    ...
    if batch_judge_json.exists():
        ...
        return {
            ...
            "message": f"배치 {current_batch_index}의 judge.json이 이미 존재합니다. 스킵합니다.",
        }
    ...
    try:
        ...
        result = run_judge_gemini(
            ...
            fusion_dir=batch_dir,  # 배치 디렉토리 사용
