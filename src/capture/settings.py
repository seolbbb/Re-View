...
class CaptureSettings:
    ...
    persistence_drop_ratio: float # 슬라이드 전환(종료) 판단을 위한 특징점 감소 비율
...
def _coerce_bool(settings: Dict[str, Any], key: str, default: bool) -> bool:
    """
    [Usage File] Internal use (settings.py)
    [Purpose] 설정 딕셔너리에서 값을 안전하게 가져와 불리언(bool)으로 변환합니다.
    [Args] settings(Dict), key(str), default(bool)
    """
    value = settings.get(key, default)
